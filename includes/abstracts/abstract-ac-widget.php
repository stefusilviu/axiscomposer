...
abstract class AC_Widget extends WP_Widget {
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			 ...
			 */
			$instance[ $key ] = apply_filters( 'axiscomposer_widget_settings_sanitize_option', $instance[ $key ], $new_instance, $key, $setting );

			/**
			 * Sanitize the value of a setting by its key.
			 */
			$instance[ $key ] = apply_filters( "axiscomposer_widget_settings_sanitize_option_$key", $instance[ $key ], $new_instance, $key, $setting );
		}
	...
	public function form( $instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			switch ( $setting['type'] ) {
				...
				default:
					do_action( 'axiscomposer_widget_field_' . $setting['type'], $key, $setting, $class, $value, $instance );
				break;
			}
		}
	}
